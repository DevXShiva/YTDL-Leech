...
async def yt_leech(client, message: Message):
    ...
    if "-n " in raw_text:
        try:
            parts = raw_text.split("-n ", 1)[1].split(None, 1)
            custom_name, url = parts[0], parts[1]
        except: pass
    ...
    try:
        with yt_dlp.YoutubeDL({'format': 'best', 'outtmpl': f'{d_dir}%(title)s.%(ext)s', 'progress_hooks': [lambda d: ytdl_hook(d, loop, status, tid)], 'quiet': True}) as ydl:
            info = ydl.extract_info(url, download=True)
            path = ydl.prepare_filename(info)
            if custom_name:
                new_path = os.path.join(d_dir, f"{custom_name}{os.path.splitext(path)[1]}")
                os.rename(path, new_path)
                path = new_path

        thumb = generate_thumbnail(path, os.path.join(d_dir, "thumb.jpg"))
        await status.edit_text("📤 Uploading...")
        sent = await message.reply_video(video=path, thumb=thumb, caption=f"✅ `{os.path.basename(path)}`", progress=progress_ui, progress_args=("Upload", tid))
